...
BRD_SER2_RTSCTS  = 0        # 不使用硬件流控，只接 TX/RX/GND
```
...
能持续打印 `mode`、`armed`、`CH5`、`CH6`、`CH7` 即表示 TELEM2 通信正常。
...
```text
SERIAL4_PROTOCOL = 9        # Lidar / Rangefinder serial protocol
SERIAL4_BAUD     = 115      # 115200 baud

RNGFND1_TYPE     = Benewake TFmini / TFmini Plus
...
不同 ArduPilot 版本中参数名可能显示为 `RNGFND1_MIN_CM`、`RNGFND1_MAX_CM` 或下拉选项文字。原则保持一致：类型选择 Benewake TFmini/TFmini Plus，方向选择 Down，串口协议为 Lidar，串口波特率为 115200。
...
EK3_SRC1_POSXY     = 0        # 无 GPS 室内环境不使用水平位置源
EK3_SRC1_VELXY     = 5        # OpticalFlow
EK3_SRC1_POSZ      = 1        # Baro 或按项目高度源设置
